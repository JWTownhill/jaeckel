...
const (
	...
	// two iterations is enough according to Jaeckel
	impliedVolatilityMaxIterations = 2

	// sqrtThree = √3
	sqrtThree = 1.7320508075688772935274463415058723669428052538103806280558069795

	// sqrtOneOverThree = 1/√3
	sqrtOneOverThree = 0.5773502691896257645091487805019574556476017512701268760186023264

	// twoPiOverSqrtTwentySeven = 2π/√27
	twoPiOverSqrtTwentySeven = 1.2091995761561452337293855050947704881893774987284937170465899569

	// sqrtThreeOverThirdRootTwoPi = √3 / ∛(2π)
	sqrtThreeOverThirdRootTwoPi = 0.9386434874273835660750513561150758784146887695739289481346864597

	// piOverSix = π/6
	piOverSix = 0.5235987755982988730771072305465838140328615665625176368291574321

	// sqrtTwoPi = √(2π)
	sqrtTwoPi = 2.5066282746310005024157652848110452530069867406099383166299235763
)
...
func ImpliedBlackVolatility(price, F, K, T float64, optionType OptionType) float64 {
	maxPrice := F
	if optionType == Put {
		maxPrice = K
	}
...
func NormalizedImpliedBlackVolatility(beta, x float64, optionType OptionType) float64 {
	thetaX := x
	if optionType == Put {
		thetaX = -x
	}
...
func lowestBranchInitialGuess(thetaX, sL, bL, beta float64) float64 {
	...
	// NOTE: This branch (f <= 0) appears unreachable in practice because the rational cubic
	// interpolation with carefully chosen control parameters preserves positivity.
	// Jäckel's original C++ comment: "This can happen due to roundoff truncation for extreme
	// values such as |x| > 500." However, testing with |x| up to 1000 has not triggered this path.
	// Kept for consistency with the original C++ implementation, and as a defensive
	// safeguard against potential numerical edge cases.
	if f <= 0 {
...
func invNormalizedVega(x, s float64) float64 {
	ax := math.Abs(x)
	// NOTE: This branch (ax <= 0) appears unreachable via the public API because:
	// - This function is only called from letsBeRational with thetaX (which is -|x|, always negative)
	// - The ATM case (x=0) is handled separately before this function is called
	// Kept for consistency with Jäckel's original C++ (SAFEGUARD_VEGA_AGAINST_ZERO_VOLATILITY),
	// and as a defensive safeguard.
	if ax <= 0 {
		return sqrtTwoPi * math.Exp(0.125*s*s)
	}
	// NOTE: The s <= 0 case is also unlikely to be reached in practice since s values
	// passed here come from valid interpolation ranges. Kept consistency with original C++ and defensiveness.
...
func inverseFLowerMap(x, f float64) float64 {
	// NOTE: This branch (f <= 0) appears unreachable in practice because f comes from
	// either the rational cubic interpolation (which preserves positivity for valid inputs)
	// or the quadratic fallback in lowestBranchInitialGuess (which also produces positive f).
	// Kept for consistency with Jäckel's original C++ (POSITIVE_DENORMALISATION_CUTOFF check),
	// and as a defensive safeguard.
	if f <= 0 {
